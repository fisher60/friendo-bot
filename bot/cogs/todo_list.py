...
async def update_of_todos(ctx: Context, todos: str) -> None:
    ...
    if TODO_FILE.is_file():  # Checks file if it exists
        ...
        if TODO_FILE.stat().st_size > 0:
            async with aiofiles.open(TODO_FILE) as read:
    ...
    # If file does not exist, a new todo list will be created
        ...
        async with aiofiles.open(TODO_FILE, "w+") as to_write:
            todo_dict = {
                ctx.author.id: {num: todo.strip().rstrip() for num, todo in enumerate(todos, start=1)}
            }
...
async def deletion_of_todos(ctx: Context, keys_to_delete: str) -> bool:
    ...
    if TODO_FILE.is_file():
        log.info("todo_list_data.json exists")
        if TODO_FILE.stat().st_size > 0:  # This will check if the file is NOT empty
            async with aiofiles.open(TODO_FILE) as read:
                todo_file_read = json.loads(await read.read())
                if (
                    str(ctx.author.id)
                    ...
                    todo_file_read[str(ctx.author.id)] = dict(
                        enumerate(todo_file_read[str(ctx.author.id)].values(), start=1)
                    )
...
class TodoList(Cog):
    ...
    async def todo_list(self, ctx: Context, *, todos: str | None = None) -> None:
    ...
    async def delete_todos(self, ctx: Context, *, todos: str | None = None) -> None:
    ...
    async def show_todos(self, ctx: Context) -> None:
        ...
        if TODO_FILE.stat().st_size > 0:
            ...
            await ctx.send(embed=Embed(title="Todo list file empty! Arf!", color=Colour.red()))
