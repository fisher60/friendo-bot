...
class Music(commands.Cog):
    ...
    async def get_lyrics(
        self, ctx: discord.ext.commands.context.Context, *, song_title: str
    ) -> None:
        ...
        try:
            ...
            embed = discord.Embed(
                title=f"{data['name']} by {data['artist']['name']}", url=data["url"]
            )
            ...
            embed.add_field(
                name="Error", value=f"Could not find song with title {song_title}"
            )
            await ctx.send(embed=embed)
    ...
    async def getalbum(
        self, ctx: discord.ext.commands.context.Context, *, album_title: str
    ) -> None:
        ...
        try:
            ...
            if "published" in wiki.keys():
                embed.add_field(
                    name="Release Data", value=wiki["published"], inline=False
                )
            else:
            ...
            embed = discord.Embed(title="Music Cog", color=discord.Colour.blue())
            embed.add_field(
                name="Error", value=f"Could not find album with title {album_title}"
            )
            await ctx.send(embed=embed)
    ...
    async def getartist(
        self, ctx: discord.ext.commands.context.Context, *, args: str
    ) -> None:
        """Function to return discord embed with artist info."""
        try:
            data = await get_artist(args)
            start_data = await get_data(
                "artist.gettopalbums&artist=" + urllib.parse.quote(data["name"])
            )
    ...
    async def topsongs(self, ctx: discord.ext.commands.context.Context) -> None:
        ...
        for count, song in enumerate(data[:10], 1):
            em.add_field(
                name=str(count),
                value=f"{song['name']} by {song['artist']['name']}",
                inline=False,
            )
...
async def get_data(url_data1: str, url_data2: str = "") -> dict:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(
            f"{url}{url_data1}&api_key={MUSIC_TOKEN}&format=json{url_data2}"
        ) as resp:
...
async def get_track(track: str, artist: str = "") -> dict:
    ...
    if artist:
        return (
            await get_data("track.getinfo", "&artist=" + artist + "&track=" + track)
        )["track"]
    else:
        artist = (await get_data("track.search&track=" + track))["results"][
            "trackmatches"
        ]["track"][0]
        track = (await get_data("track.search&track=" + track))["results"][
            "trackmatches"
        ]["track"][0]
