...
class Fun(Cog):
    ...
    @command(brief="Send a number and get the factorial of it")
    async def factorial(self, ctx: Context, number: int) -> None:
        """Sends the factorial of the user-inputted number."""
        if number > 69:
    ...
    async def spongify(self, ctx: Context, *, phrase: str) -> None:
    ...
    @command(
        ...
        description="accepts 'heads' or 'tails' and tells if the output matches the user input.",
        name="flip",
    )
    async def coin_toss(self, ctx: Context, toss: str) -> None:
    ...
    async def rps(self, ctx: Context, *, response: str) -> None:
