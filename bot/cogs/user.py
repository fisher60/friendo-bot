...
BADGES = {
    "hypesquad_bravery": "<:bravery:785001148453093386>",
...
class User(Cog):
    ...
    async def userinfo(self, ctx: Context, member: Union[str, Member] = None) -> None:
        ...
        create_time = ", ".join(
            self.get_timedelta(datetime.datetime.utcnow(), user.created_at)
        )
        joined_time = ", ".join(
            self.get_timedelta(datetime.datetime.utcnow(), user.joined_at)
        )
        ...
        info_emb.add_field(name="Status", value=statuses, inline=False)
        info_emb.add_field(name="Activity", value=activity, inline=False)
